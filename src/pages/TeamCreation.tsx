...
export const TeamCreation: React.FC = () => {
	...
	const [phase, setPhase] = React.useState<Phase>(Phase.Setup);
	const [confirmed, setConfirmed] = React.useState<PlayerResult[]>([]);
	const [usedLanes, setUsedLanes] = React.useState<Lane[]>([]);
	const [currentIdx, setCurrentIdx] = React.useState(0);
	const [drawStep, setDrawStep] = React.useState<DrawStep>(DrawStep.Lane);
	const [pendingLane, setPendingLane] = React.useState<Lane | null>(null);
	const [confirmedLane, setConfirmedLane] = React.useState<Lane | null>(null);
	const [pendingChampion, setPendingChampion] = React.useState<Champion | null>(
		null,
	);
	...
	const handleSortearLane = () => {
		const available = Object.values(Lane).filter((l) => !usedLanes.includes(l));
	...
	return (
		<>
			{phase === Phase.Setup && (
				<section className="players-form">
					{players.map((p, i) => (
						<input
							key={i}
							type="text"
				...
				<section className="drawing-phase">
							...
							{confirmed.map((r, i) => (
								<div key={i} className="result-card result-card--done">
									...
									<span className="result-lane">
										<LaneIcon iconName={r.lane} /> {laneLabels[r.lane]}
									</span>
					...
					<div className="draw-card">
								...
								{pendingLane ? (
									<>
										<span className="draw-value">
											<LaneIcon iconName={pendingLane} />{" "}
											{laneLabels[pendingLane]}
										</span>
						...
						{drawStep === DrawStep.Champion && confirmedLane && (
							<section className="draw-step">
								<span className="draw-confirmed-lane">
									<LaneIcon iconName={confirmedLane} />
